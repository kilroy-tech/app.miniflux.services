/**
 * miniflux.services/chat_agent/stop_hook.js - preflight/postflight functions for miniflux.services/chat_agent/stop_hook
 * @module miniflux.services/chat_agent/stop_hook
 * @file miniflux.services/chat_agent/stop_hook preflight/postflight implementation
 * @author system
 * @copyright Copyright ©2024, Concluent Systems, LLC. All rights reserved.
 */
"use strict";
const MODULE_NAME = "workflow:miniflux.services/chat_agent/stop_hook";
...
function postflight(authData, wfProxy) {
    debug ("postflight");
    let ta_alias = wfProxy.getGlobalValue ("ta_alias") || "undefined_bot";
    let bot = global.REBAR_NAMESPACE.__Miniflux [ta_alias];
    
    try {
        return bot.bot.stop ().then (res => {
            debug (`stopped bot ${ta_alias}`);
            delete global.REBAR_NAMESPACE.__Miniflux [ta_alias];
            return Promise.resolve ({success:true});
        });
    }
    catch (err) {
        debug (`error stopping bot ${ta_alias}: ${err}`);
    }
    return Promise.resolve({success: true});
}
