/**
 * miniflux.services/chat_agent/swarm_hook.js - preflight/postflight functions for miniflux.services/chat_agent/swarm_hook
 * @module miniflux.services/chat_agent/swarm_hook
 * @file miniflux.services/chat_agent/swarm_hook preflight/postflight implementation
 * @author system
 * @copyright Copyright ©2025, Concluent Systems, LLC. All rights reserved.
 */
"use strict";
const MODULE_NAME = "workflow:miniflux.services/chat_agent/swarm_hook";
...
const MinifluxifyMarkdown = require('Minifluxify-markdown');
...
function preflight(authData, wfProxy) {
    ...
    try {
        ...
        let bot = global.REBAR_NAMESPACE.__Miniflux [ta_alias];
        let chat_id = bot.chat_id;
        
        let use_markdown = bot.use_markdown;
        let parse_mode = {};

        if (from != ta_agent_from && ta_agent_from != "*") { //doesn't match who we are listening for... don't send to Miniflux
debug (`not sending msg to Miniflux from: ${from}`);
